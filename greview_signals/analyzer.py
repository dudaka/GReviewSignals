...
This module contains the core GoogleBusinessProfileAnalyzer class for analyzing
Google Business Profile reviews from Google Takeout data. It provides functionality
to extract person names mentioned in reviews using Named Entity Recognition (NER)
and offers comprehensive filtering and export capabilities.

The analyzer can:
- Load reviews from Google Takeout JSON files
- Filter reviews by date range and star ratings
- Extract person names using spaCy NLP
...
Example:
    ...
    analyzer = GoogleBusinessProfileAnalyzer(data_dir="data", output_dir="output")
    filtered_reviews, name_counts = analyzer.analyze(
        year=2025, 
        star_ratings=['FOUR', 'FIVE'],
        export_names_csv="staff_mentions.csv"
    )
